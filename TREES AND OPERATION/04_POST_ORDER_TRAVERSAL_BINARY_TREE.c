...
/* THERE ARE 3 WAYS TO TRAVRESE A NON LINAR STRUCTURE
    ...
    IN THIS SECTION, WE WILL SEE THE POST ORDER TRAVERSAL OF OUR BINARY TREE

    IN THE POST ORDER TRAVERSAL, THE PRIORITY OF TRAVERSING IS :
...
int POST_ORDER( struct Node * root ){
    ...
    if( root != NULL ){
        ...
        POST_ORDER( root->left); // left is first

        POST_ORDER( root->right);// right is next

        printf("  %d    ",root->data); // root is last
    }
